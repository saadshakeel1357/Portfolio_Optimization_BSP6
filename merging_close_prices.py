...
"""
This script reads multiple CSV files containing close prices for different financial instruments,
...
It assumes the first row of each CSV file contains a header and the second row contains the ticker symbol."""



def extract_ticker(filepath):
    """
    Read the second line of the CSV file (comma-separated) and extract the ticker symbol.
    """
    with open(filepath, 'r') as f:
        f.readline()               # skip header line
        second_line = f.readline().strip()
    # assume format: "Ticker,<SYMBOL>"
    return second_line.split(',')[1]


def load_close_series(filepath):
    """
    Load a CSV file, keep only the 'close' column, rename it to the ticker.
    """
...
def main():
    ...
    # save the merged data to CSV without the header row
