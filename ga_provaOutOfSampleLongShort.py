...
for kappa in range(kmax-kmin):
    ...
    pd.DataFrame(two_sided_rolling).iloc[:,kappa].to_excel('K'+str(K_u)+'Fitness_TwosidedNIKKEILO.xlsx',index=None)
    pd.DataFrame(two_sided_rolling_entropy).iloc[kappa].to_excel('K'+str(K_u)+'Entropy_TwosidedNIKKEILO.xlsx',index=None)
